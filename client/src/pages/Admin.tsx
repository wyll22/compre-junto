...
type AdminTab = "products" | "groups" | "banners" | "videos";
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                data-testid="input-now-price"
                type="number"
                step="0.01"
                value={form.nowPrice}
                onChange={(e) => setForm({ ...form, nowPrice: e.target.value })}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                data-testid="input-reserve-fee"
                type="number"
                step="0.01"
                value={form.reserveFee}
                onChange={(e) => setForm({ ...form, reserveFee: e.target.value })}
              />
            </div>
          </div>
  ...
  useEffect(() => {
    if (editBanner) {
      setForm({
        ...
        mobileImageUrl: editBanner.mobileImageUrl || "",
        linkUrl: editBanner.linkUrl || "",
        sortOrder: String(editBanner.sortOrder || 0),
        active: editBanner.active !== false,
      });
    } else {
...
function VideoForm({
  ...
  useEffect(() => {
    if (editVideo) {
      setForm({
        ...
        embedUrl: editVideo.embedUrl || "",
        sortOrder: String(editVideo.sortOrder || 0),
        active: editVideo.active !== false,
      });
    } else {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !mutation.isPending && !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{editVideo ? "Editar Video" : "Novo Video"}</DialogTitle>
        </DialogHeader>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <div className="space-y-1.5">
            <Label>URL do Video (embed)</Label>
            <Input value={form.embedUrl} onChange={(e) => setForm({ ...form, embedUrl: e.target.value })} required placeholder="https://youtube.com/embed/..." />
          </div>
          <div className="space-y-1.5">
            <Label>Ordem</Label>
            <Input type="number" value={form.sortOrder} onChange={(e) => setForm({ ...form, sortOrder: e.target.value })} />
    ...
    <div className="min-h-screen bg-background">
      <header className="brand-gradient border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between gap-2">
          ...
          <Link href="/" className="text-sm font-medium text-white/80 hover:text-white flex items-center gap-1">
                        ...
                        products?.map((product: any) => (
                          <TableRow key={product.id}>
                            ...
                            <TableCell className="text-right text-sm">
                              {product.nowPrice ? `R$ ${Number(product.nowPrice).toFixed(2)}` : "-"}
                            </TableCell>
                                  ...
                                  <AlertDialogContent>
                                    <AlertDialogHeader>
                                      ...
                                      <AlertDialogDescription>
                                        Essa acao nao pode ser desfeita. O produto "{product.name}" sera excluido permanentemente.
                                      </AlertDialogDescription>
                                    </AlertDialogHeader>
                                    <AlertDialogFooter>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {!allGroups || allGroups.length === 0 ? (
                        ...
                        (allGroups as any[]).map((group) => (
                          <TableRow key={group.id}>
                            <TableCell className="text-sm">{group.id}</TableCell>
                            <TableCell className="text-sm">{group.productId}</TableCell>
                            <TableCell className="text-sm">{group.currentPeople}</TableCell>
                            <TableCell className="text-sm">{group.minPeople}</TableCell>
                            <TableCell>
                              <Badge
                                variant={group.status === "aberto" ? "default" : "secondary"}
                                className="text-[10px]"
                              >
                                {group.status}
                              </Badge>
                            ...
                            <TableCell className="text-right">
                              {group.status === "aberto" ? (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => updateGroupStatus.mutate({ id: group.id, status: "fechado" })}
                                >
                                  Fechar
                                </Button>
                              ) : (
                                <Button
                                  variant="outline"
                                  size="sm"
                                  onClick={() => updateGroupStatus.mutate({ id: group.id, status: "aberto" })}
                                >
                                  Reabrir
                                </Button>
                              )}
          ...
          <>
            ...
            <div className="space-y-3">
              {!banners || banners.length === 0 ? (
                ...
                (banners as any[]).map((banner) => (
                  <Card key={banner.id}>
                    <CardContent className="p-3 flex items-center gap-3">
                      <img
                        src={banner.imageUrl}
                        alt={banner.title}
                        className="w-24 h-14 rounded-md object-cover bg-muted"
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = "https://via.placeholder.com/96x56";
                        }}
                      />
                      ...
                      <Badge variant={banner.active ? "default" : "secondary"} className="text-[10px]">
